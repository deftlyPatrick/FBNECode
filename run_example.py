...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        ...
        for test_u, test_v, tmp_target in test_loader:
            ...
            # print("\npred: ", tmp_pred[counter][0:20])

            target.append(list(tmp_target.data.cpu().numpy()))
            # target[counter].sort(reverse=True)
            # print("target: ", target[counter][0:20], "\n")
            # counter += 1
...
def load(path):
    ...
    for net_type in ['yelp_academic_dataset_business_total_dataset_big_u2u',
                     'yelp_academic_dataset_business_total_dataset_big_u2b']:
        with open(path + net_type + ".net") as fp:
            for line in fp:
                info = line.strip().split("`t")
                if net_type == 'yelp_academic_dataset_business_total_dataset_big_u2u':
                    node1 = info[1]
                    node2 = info[2]
                    node2 = node2.replace("[", "")
                    node2 = node2.replace("]", "")
